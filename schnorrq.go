...
func init() {
	...
	// Set LD_LIBRARY_PATH relative to the directory containing the shared library
	libPath := filepath.Join(dir, "libfourq-qubic.so")
	fmt.Println(libPath)
	...
	if currLDPath == "" {
		...
		os.Setenv("LD_LIBRARY_PATH", currLDPath+":"+libPath)
	}
}
